...
CORS(app, resources={
    r"/api/*": {
        "origins": [
            ...
            "http://127.0.0.1:3000",
            "http://127.0.0.1:3001",
            "http://100.25.54.28:3000",
            "http://100.25.54.28:3001",
            "http://100.25.54.28"
        ],
...
# Add default route and health check
@app.route('/')
def index():
    return jsonify({
        "status": "ok",
        "message": "Disaster Feed API is running",
        "version": "1.0.0",
        "endpoints": {
            "health": "/health",
            "posts": "/api/posts",
            "disaster_types": "/api/disaster-types",
            "disaster_summary": "/api/disaster-summary",
            "charts": {
                "disaster_distribution": "/api/chart/disaster-distribution",
                "disaster_timeline": "/api/chart/disaster-timeline",
                "post_volume": "/api/chart/post-volume-metrics"
            }
        }
    })
...
def health_check():
    try:
        ...
        dynamodb = get_dynamodb()
        dynamodb.meta.client.describe_table(TableName=POSTS_TABLE)
