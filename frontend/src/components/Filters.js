...
function Filters({ setSelectedDisaster, selectedDisaster, availableTypes = [] }) {
    ...
    const getCategoriesToDisplay = (availableTypes) => {
        // Ensure availableTypes is an array
        const types = Array.isArray(availableTypes) ? availableTypes : [];
        
        return DISASTER_CATEGORIES.filter(category => 
            types.some(type => type.includes(category.value))
        );
    };
